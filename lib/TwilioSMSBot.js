...
function TwilioSMS (configuration) {
  ...
  if (!configuration) {
    throw Error(`Specify your 'account_sid', 'auth_token', ` +
      `'twilio_number', and 'server_url' as properties of the 'configuration' object`)
  }
  ...
  if (configuration && !configuration.server_url) {
    throw Error(`Specify a 'server_url'`)
  }
  ...
  twilioSMS.defineBot(function (botkit, config) {
    ...
    bot.send = function (message, cb) {
      ...
      let sms = {
        ...
        to: message.channel,
        statusCallback: configuration.server_url + '/sms/status'
      }
  ...
  twilioSMS.createWebhookEndpoints = function (webserver, bot, cb) {
    ...
    webserver.post('/sms/status', function (req, res) {
      if (req.body.MessageStatus === 'undelivered' ||
